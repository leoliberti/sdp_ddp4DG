...
## read command line
if len(sys.argv) < 2:
    exit('cmdline must be: filename.dat [plot]')
...
cobj3 = cp.trace(R@X)
cobj = cobj1 + 0.1*cobj2 + 0.01*cobj3
#objective = cp.Minimize(cobj)
objective = cp.Minimize(cobj1 + 0.1*cobj2)
...
writeRlz(n,K,x,rlzbase + "-sol.dat")
